...
public class BarChartRenderer extends BarLineScatterCandleBubbleRenderer {
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        for (int j = 0; j < buffer.size(); j += 4) {
         ...
         if (dataSet.isConnerRadius()){
             RectF rectF=new RectF(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
                     buffer.buffer[j + 3]);
             float connerRadius = (buffer.buffer[j + 2] - buffer.buffer[j])/2;
             c.drawRoundRect(rectF,connerRadius,connerRadius,mRenderPaint);
         }else{
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (Highlight high : indices) {
           ...
           // c.drawRect(mBarRect, mHighlightPaint);
            ...
            if (set.isConnerRadius()){
                float connerRadius = (mBarRect.right- mBarRect.left)/2;
                c.drawRoundRect(mBarRect,connerRadius,connerRadius,mHighlightPaint);
            }else{
                c.drawRect(mBarRect, mHighlightPaint);
            }
